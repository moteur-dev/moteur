...
import { getJson, putJson, deleteKey } from './utils/storageAdapterUtils.js';
...
export async function getSchedule(
    ...
    const storage = getProjectStorage(projectId);
    const schedule = await getJson<Schedule>(storage, scheduleKey(scheduleId));
    if (!schedule) {
...
export async function createSchedule(
    ...
    const storage = getProjectStorage(projectId);
    await putJson(storage, scheduleKey(id), schedule);
...
export async function cancelSchedule(
    ...
    await putJson(storage, scheduleKey(scheduleId), updated);
...
export async function deleteSchedule(
    ...
    await deleteKey(storage, scheduleKey(scheduleId));
