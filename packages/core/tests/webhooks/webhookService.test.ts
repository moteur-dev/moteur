...
describe('webhookService', () => {
    ...
    describe('delivery log cap (500)', () => {
        it('trims to 500 entries per project', async () => {
            ...
            const { getJson, putJson } = await import('../../src/utils/storageAdapterUtils.js');
            const { getProjectStorage } = await import('../../src/utils/getProjectStorage.js');
            ...
            await putJson(storage, WEBHOOK_LOG_KEY, existing);
            ...
            const raw = await getJson<any[]>(storage, WEBHOOK_LOG_KEY);
