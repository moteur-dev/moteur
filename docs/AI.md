...
Moteur uses a **provider-agnostic AI layer** implemented in the `@moteur/ai` package. The API and Studio consume this layer for:
...
- **Field generation** — generate field definitions from a prompt (admin)
- **Image generation** — generate images from a text prompt (when the provider supports it)

All AI operations (except admin-only field generation) use a **project-level credit system**. Credits are deducted before each call; when the balance is insufficient, the API returns `402 Payment Required` with `error: 'insufficient_credits'`.

---

## Package: @moteur/ai

The core AI logic lives in **`packages/ai`** and is published as **`@moteur/ai`**. It provides:
...
The **API** (`@moteur/api`) depends on `@moteur/ai` and uses it for all AI routes. The API re-exports `getAdapter`, `setAdapter`, and the credit helpers so existing tests and callers can keep using the same imports; types are imported from `@moteur/ai`.
...
| **`MOTEUR_AI_API_KEY`** | API key for the selected provider. Can be overridden by provider-specific keys below. |
...
- **Requires:** `OPENAI_API_KEY` or `MOTEUR_AI_API_KEY` when `MOTEUR_AI_PROVIDER=openai`.
- **Capabilities:** `generate`, `generateStructured`, `embed`, `analyseImage`, `generateImage`.

### Anthropic (`anthropic`)

- **Model:** `claude-sonnet-4-20250514`.
- **Requires:** `ANTHROPIC_API_KEY` or `MOTEUR_AI_API_KEY` when `MOTEUR_AI_PROVIDER=anthropic`.
