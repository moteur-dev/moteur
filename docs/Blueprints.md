...
Blueprints are **global**; no project ID is required. All under the same auth as other admin endpoints (e.g. bearer token). The API uses **path-based** routes by kind.
