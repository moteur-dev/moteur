...
router.post('/', requireAuth, async (req: any, res: any) => {
    ...
    const io = (req.app as any).locals?.io as
    ...
    if (entryId && fieldPath && modelId && io) {
        ...
        const captionGranted = presenceStore.tryLockField(
            projectId,
            captionSubField,
            AI_LOCK_USER_ID,
            {
        ...
        try {
            ...
            if (arrayMatch) {
                ...
                const item =
                    typeof arr[index] === 'object' && arr[index] !== null
                ...
                if (altGranted) {
                    const altMap = (
                        typeof fieldValue.alt === 'object' && fieldValue.alt !== null
                            ? { ...(fieldValue.alt as Record<string, string>) }
                            : {}
                ...
                if (captionGranted) {
                    const captionMap = (
                        typeof fieldValue.caption === 'object' && fieldValue.caption !== null
                            ? { ...(fieldValue.caption as Record<string, string>) }
                            : {}
                    ) as Record<string, string>;
