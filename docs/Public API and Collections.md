...
Each project has at most one API key. You create it via the **Admin API** (with a JWT).
...
2. **Generate the key** for your project:
   ```http
   POST /admin/projects/:projectId/api-key/generate
   Authorization: Bearer <your-jwt>
   ```
   Response: `{ "prefix": "mk_live_...", "rawKey": "mk_live_xxxxxxxx...", "message": "Store this key securely. It will not be shown again." }`.

3. **Store the `rawKey`** in a safe place (env var, secrets manager). You will **never** see it again; only the prefix is shown later (e.g. `GET /admin/projects/:projectId/api-key`).

---

## 2. Create a collection

A collection defines *what* that API key can read: which models and pages, and with which options (fields, status filter, reference resolution).
...
**Query:**
```http
GET /projects/:projectId/collections?apiKey=<your-raw-key>
```
...
- **Base path:** If your API uses a base path (e.g. `API_BASE_PATH=/api`), prefix all paths with it: `/api/projects/...`, `/api/admin/...`.
- **404:** If the URL’s `resourceId` or `collectionId` is not in the collection, you get **404**.

For full endpoint and option details, see [REST API](REST%20API.md). For programmatic access (Node), see [Developer API](Developer%20API.md).
