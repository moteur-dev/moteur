...
export async function listTemplates(projectId: string): Promise<TemplateSchema[]> {
    ...
    const raw = await storage.list(templateListPrefix());
    const ids = parseTemplateIds(raw);
    const schemas: TemplateSchema[] = [];

    for (const id of ids) {
        const schema = await getJson<TemplateSchema>(storage, templateKey(id));
...
export async function createTemplate(
    ...
    const storage = getProjectStorage(projectId);
    const exists = await hasKey(storage, templateKey(id));
    if (exists) {
    ...
    await putJson(storage, templateKey(id), schema);
...
export async function updateTemplate(
    ...
    const storage = getProjectStorage(projectId);
    await putJson(storage, templateKey(id), updated);
