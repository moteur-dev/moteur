...
async function loadSchedule(projectId: string, scheduleId: string): Promise<Schedule | null> {
    ...
    return getJson<Schedule>(storage, scheduleKey(scheduleId));
}

async function saveSchedule(schedule: Schedule): Promise<void> {
    const storage = getProjectStorage(schedule.projectId);
    await putJson(storage, scheduleKey(schedule.id), schedule);
}
    ...
    try {
        ...
        for (const project of projects) {
            try {
                ...
                for (const id of ids) {
                    try {
                        const schedule = await getJson<Schedule>(storage, scheduleKey(id));
