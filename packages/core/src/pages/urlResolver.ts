...
export function interpolatePattern(pattern: string, entry: Entry): string {
    const re = /\[([^\]]+)\]/g;
    return pattern.replace(re, (_, path: string) => {
        const value = getDataByPath(entry.data ?? {}, path.trim());
        return value;
    });
}
...
export async function resolveAllUrls(
    ...
    const entryFetches = collectionNodes.map(cn =>
        getEntries(projectId, cn.modelId, cn.entryStatus ?? 'published')
    );
    const entryLists = await Promise.all(entryFetches);

    for (const node of nodes) {
        if (node.type === 'folder') continue;
        if (node.type === 'static') {
            const sn = node as StaticPage;
            if (sn.status !== 'published') continue;
            const prefix = resolveNodePrefix(node.id, nodeMap);
            emit(sn, prefix || '/');
            continue;
        }
