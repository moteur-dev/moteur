...
import { Field } from '../../../../../src/types/Field.js';
...
describe('validateTextField', () => {
  ...
  it('returns error for non-string value', () => {
    ...
    expect(issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'error', message: 'Expected a string value.' })
      ])
    );
  });
  ...
  it('returns error for string shorter than minLength', () => {
    ...
    expect(issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'error', message: 'Value is too short (min 3 chars).' })
      ])
    );
  });
  ...
  it('returns error for string longer than maxLength', () => {
    ...
    expect(issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'error', message: 'Value is too long (max 10 chars).' })
      ])
    );
  });
  ...
  it('returns error for string not matching pattern', () => {
    ...
    expect(issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'error', message: expect.stringContaining('does not match pattern') })
      ])
    );
  });
});
